...
public class MainGUI extends JFrame {
    ...
    private void handleSystemOfEquations(String input) {
        if (step == 1) {
            ...
            currentSystem = RandomLinearSystemGenerator.generateRandomLinearSystem(numEquations, numVariables);
            double[][] coefficients = currentSystem.getCoefficients();
            double[] constants = currentSystem.getConstants();
            outputArea.append("Generated system of equations:\n");
            for (int i = 0; i < numEquations; i++) {
                StringBuilder equation = new StringBuilder();
                for (int j = 0; j < numVariables; j++) {
                    double coeff = coefficients[i][j];
                    if (coeff != 0) {
                        if (equation.length() > 0 && coeff > 0) {
                            equation.append(" + ");
                        } else if (coeff < 0) {
                            equation.append(" - ");
                            coeff = -coeff;
                        }
                        equation.append(String.format("%.2f x%d", coeff, j + 1));
                    }
                }
                equation.append(String.format(" = %.2f\n", constants[i]));
                outputArea.append(equation.toString());
            }
            double[] solution;
            String methodUsed;
            if (numEquations < 100) {
                solution = GaussianElimination.solve(currentSystem);
                methodUsed = "GaussianElimination";
            } else {
                solution = GaussSeidel.solve(currentSystem, 1e-6, 1000);
